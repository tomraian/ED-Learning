<footer class="footer">
    <div class="container">
        <div class="footer__top">
            <div class="row">
                <a href="" class="back-to-top">
                    <div class="line"></div>
                    CUỘN LÊN
                </a>
                <div class="col-lg-5 col-md-6">
                    <div class="footer__left">
                        <div class="footer__decs">
                            <p>
                                Sáng tạo, tinh tế và phù hợp sẽ khiến sản phẩm của bạn trở nên khác biệt.</p>
                        </div>
                        <div class="footer__contact">
                            <ul>
                                <li>Số 11, Phan Kế Bính, Quận 1, TP. Hồ Chí Minh</li>
                                <li>(+84) 98 9596 913</li>
                            </ul>
                        </div>
                        <div class="footer__social">
                            <a href="https://www.facebook.com/profile.php?id=100034249281118" target="new-tab">
                                <img src="img/facebook.svg" alt="" class="svg">
                            </a>
                            <a href="">
                                <img src="img/skype.svg" alt="" class="svg">
                            </a>
                            <a href="">
                                <img src="img/email.svg" alt="" class="svg">
                            </a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="footer__right">
                        <ul class="footer__right-list">
                            <li class="footer__list-items"><a href="index.php">Trang chủ</a></li>
                            <li class="footer__list-items"><a href="courses.php">Khóa học</a></li>
                            <li class="footer__list-items"><a href="team.php">ED Team </a></li>
                            <li class="footer__list-items"><a href="policy.php">Điều khoản</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="copyright">
        <div class="container">
            <div class="decs">
                <p>2021 Creative Front-End Dev</p>
            </div>
        </div>
    </div>
</footer>
</div>
<script type="text/javascript" src="dest/jsmain.min.js"></script>
<script type="text/javascript" src="js/main.js"></script>
</body>

</html>